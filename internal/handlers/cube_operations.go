...
func HandleCommitCube(w http.ResponseWriter, r *http.Request) {
	log.Printf("[COMMIT-CUBE] Starting commit request at %s", time.Now().UTC().Format(time.RFC3339))

	if r.Method != http.MethodPost {
		log.Printf("[COMMIT-CUBE] Error: Invalid method %s used instead of POST", r.Method)
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	cubeIDStr := r.URL.Query().Get("cube_id")
	if cubeIDStr == "" {
		log.Printf("[COMMIT-CUBE] Error: No cube ID provided in request")
		http.Error(w, "Missing cube ID", http.StatusBadRequest)
		return
	}
	...
	cubeID, err := strconv.Atoi(cubeIDStr)
	if err != nil {
		log.Printf("[COMMIT-CUBE] Error: Invalid cube ID format: %s - %v", cubeIDStr, err)
		http.Error(w, "Invalid cube ID", http.StatusBadRequest)
		return
	}
	...
	if newImage == "" || tag == "" {
		log.Printf("[COMMIT-CUBE] Error: Missing image name or tag in request")
		http.Error(w, "Missing new image or tag", http.StatusBadRequest)
		return
	}
	log.Printf("[COMMIT-CUBE] Processing commit for cube ID: %d with image: %s and tag: %s", cubeID, newImage, tag)

	container, err := database.GetCubeData(cubeID)
	if err != nil {
		log.Printf("[COMMIT-CUBE] Database error while fetching cube data: %v", err)
		http.Error(w, fmt.Sprintf("Failed to get cube data: %v", err), http.StatusInternalServerError)
		return
	}
	log.Printf("[COMMIT-CUBE] Successfully retrieved cube data for ID: %d", cubeID)

	err = docker.CommitContainer(container.Name, newImage, tag)
	if err != nil {
		log.Printf("[COMMIT-CUBE] Docker error while committing container: %v", err)
